...
Enter causality. Causal inference (CI) focuses on formalizing these questions and developing tools to answer them in a principled way. For example by systematically identifying failure modes or by isolating the model components that cause problematic behavior we can gain insights into the causal factors that affect the model's behavior. 
This can lead to more interpretable and explainable models, while also promising to improve model generalization and robustness by avoiding falling prey to spurious correlations and biases in the data.
...
By engaging both theoretical and applied perspectives, we aim to foster deep insights into the behavior of large models, both to improve the model's behavior but also to make the behavior more interpretable and reliable.
